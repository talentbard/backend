...
class TalentFetchAssignmentView(APIView):
    @swagger_auto_schema(
        operation_description="Fetch the saved assignment question for a user.",
            ...
            200: openapi.Response(
                ...
                openapi.Schema(
                    ...
                    properties={
                        ...
                        "payload": openapi.Schema(type=openapi.TYPE_OBJECT, description="Saved assignment"),
                    },
                ),
            ),
    ...
    def post(self, request):
        ...
        try:
            assignment = GeneratedAssignment.objects.get(user=user)
            return Response({
                "message": "Assignment retrieved successfully",
                "payload": assignment.assignment_task,
                "status": 200
            }, status=status.HTTP_200_OK)
        except GeneratedAssignment.DoesNotExist:
            return Response({"message": "No assignment found for this user", "status": 404}, status=status.HTTP_404_NOT_FOUND)